...
import { PROVIDER_SCHEMA, DEBUG, MESSAGES } from './constants';
import * as stylus from 'stylus';
import { tagged, stylusChannel as log } from './Logger'
...
const StylusProvider = new class implements vscode.TextDocumentContentProvider 
{
    ...
    {
        provLog.debugLog('StylusProvider#provideTextDocumentContent called.')
        const path = uri.path;
    ...
    static async create(sourceEditor?: vscode.TextDocument)
    {
        ...
        if(!isStylusEditor(sourceEditor))
        {
            ...
            instanceTag('Failed stylus watcher initialization method, ' + 
                        'throwing error '+ MESSAGES.WATCHER.CREATION.ERROR);
            vscode.window.showErrorMessage(MESSAGES.WATCHER.CREATION.ERROR);
            return
        }
